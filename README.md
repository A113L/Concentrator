...
```
usage: concentrator_v1.3.0.py [-h] [-t TOP_RULES] [-o OUTPUT_FILE] [-m MAX_LENGTH] [-s] [-g] [-n COMBO_TARGET] [-l COMBO_LENGTH [COMBO_LENGTH ...]]
                              [-gc COMBO_OUTPUT_FILE] [--temp-dir TEMP_DIR] [--in-memory] [-cb CLEANUP_BIN] [-ca CLEANUP_ARG]
                              paths [paths ...]

Extracts top N rules sorted by raw frequency, statistical probability, or generates VALID combinatorial rules, with optional post-processing cleanup.
...
options:
  ...
  -s, --statistical_sort
                        Sorts rules by Markov sequence probability instead of raw frequency.
